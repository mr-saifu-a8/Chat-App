...
//     });
//     if (data.success) {
//       // Dono cases mein frontend se hata do
//       setMessages((prev) => prev.filter((msg) => msg._id !== messageId));
//       toast.success(
...
export const ChatProvider = ({ children }) => {
  ...
  const deleteMessage = async (messageId, deleteType) => {
    try {
      ...
      if (data.success) {
        // Dono cases mein frontend se remove karo
        setMessages((prev) => prev.filter((msg) => msg._id !== messageId));
        toast.success(
          deleteType === "forEveryone"
            ? "Message deleted for everyone"
  ...
  // ──────────────────────────────────────────
  const bulkDeleteMessages = async (messageIds, deleteType = "forMe") => {
    try {
      const { data } = await axios.delete("/api/messages/bulk-delete", {
        data: { messageIds, deleteType },
      });
      if (data.success) {
        // Frontend se sab selected messages remove karo
        setMessages((prev) =>
