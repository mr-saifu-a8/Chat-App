...
//   const { messages, isTyping } = useContext(ChatContext);
//   const scrollEnd = useRef();
//   const containerRef = useRef(); // ← container ref
...
//   // ── New conversation pe instant scroll (no animation) ──
//   useEffect(() => {
//     if (!scrollEnd.current) return;
...
const MessageList = ({ onReply, isSelecting, selectedIds, onSelect }) => {
  ...
  useEffect(() => {
    ...
    if (isFirstLoad.current) {
      // Pehli baar — instant scroll, no animation
      scrollEnd.current.scrollIntoView({ behavior: "instant" });
  ...
  // isTyping pe scroll nahi — yeh remove kiya
  // typing indicator ke liye scroll nahi hoga
